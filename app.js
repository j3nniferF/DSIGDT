...
const UI_TEXT = {
    SHIT_MODE: {
        ...
        TASK_PLACEHOLDER_ERROR: '⚠️ ADD SOME SHIT FIRST!',
    ...
    PG_MODE: {
        ...
        APP_TITLE: 'SMART stuff I NEED To Do TODAY',
...
function initializeTasks() {
    ...
    function addTask() {
        ...
        if (taskText === '') {
            ...
            const errorPlaceholder = isPgMode ? UI_TEXT.PG_MODE.TASK_PLACEHOLDER_ERROR : UI_TEXT.SHIT_MODE.TASK_PLACEHOLDER_ERROR;
            const normalPlaceholder = isPgMode ? UI_TEXT.PG_MODE.TASK_PLACEHOLDER : UI_TEXT.SHIT_MODE.TASK_PLACEHOLDER;
...
async function fetchRewardGif(searchTerm = 'excited celebration') {
    ...
    if (typeof CONFIG === 'undefined' || !CONFIG.GIPHY_API_KEY || CONFIG.GIPHY_API_KEY === 'YOUR_API_KEY_HERE') {
...
async function applyThemeText(mode) {
    ...
    if (mode === 'pg') {
        ...
        if (prizeGif) {
            prizeGif.src = gifUrl || 'https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExbHBybnhveG4wdnRodGg2MnJ1NWhxNmxzcWV5Zm4weDcyZGFqMDV1cyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/75mBr8CLHect4tHlMb/giphy.gif';
        }
        ...
        if (prizeNote) prizeNote.textContent = 'or you can stare at this cute dumb cat';
        if (backToItBtn) backToItBtn.textContent = 'NOW GET BACK TO WORK!';
        
        // Try to fetch a GIF from GIPHY, fallback to static if unavailable
        const gifUrl = await fetchRewardGif('hell yeah you did it');
        if (prizeGif) {
            prizeGif.src = gifUrl || 'https://media.giphy.com/media/uF4QwYRpMDuGuMXL1G/giphy.gif';
        }
